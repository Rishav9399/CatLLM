...
def get_system_info() -> str:
    ...
    return f"Enviornment: {os_info} on {arch}, Python Version: {py_version}"
...
def read_local_file(file_path: str) -> str:
    ...
    """
    # Security/Stability constraints: We ensure the path exists
    if not os.path.exists(file_path):
        return f"Eror: File '{file_path}' does not exist on the host machine."
    
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            # We cap the read at 8000 characters so the Swarm doesn't accidently
